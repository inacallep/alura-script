javascript:(function () {
  let taskScriptEnabled = false;

  const toggleButton = document.createElement('button');
  toggleButton.textContent = '▶ Ativar script';
  ...
  toggleButton.onclick = function () {
    taskScriptEnabled = !taskScriptEnabled;
    toggleButton.textContent = taskScriptEnabled ? '⏸ Desativar script' : '▶ Ativar script';
    toggleButton.style.background = taskScriptEnabled ? '#dc3545' : '#0d6efd';
  };
  ...
  Object.assign(modal.style, {
    ...
    backgroundColor: 'rgba(0,0,0,0.6)', display: 'none', justifyContent: 'center', alignItems: 'center'
  });

  const content = document.createElement('div');
  Object.assign(content.style, {
    position: 'relative', background: 'white', width: '80vw', height: '80vh', boxShadow: '0 0 10px black'
  });
  ...
  Object.assign(close.style, {
    position: 'absolute', top: '10px', right: '10px', zIndex: 10000, fontSize: '18px'
  });
  ...
  async function loadIntoIframe(url) {
    try {
      ...
      Array.from(doc.querySelectorAll('a')).forEach(a => {
        ...
        a.setAttribute('onclick', 'event.preventDefault(); window.parent.__loadIframe && window.parent.__loadIframe(this.href);');
      });
      ...
      iframe.srcdoc = `<p style="color:red">Erro ao carregar ${url}: ${e.message}</p>`;
      openModal();
    }
  }

  window.__loadIframe = loadIntoIframe;

  const observer = new MutationObserver(mutations => {
    mutations.forEach(m => {
      if (m.type === 'attributes' && m.attributeName === 'src') {
        const attemptedSrc = iframe.getAttribute('src');
        if (attemptedSrc && attemptedSrc !== 'about:blank') {
          iframe.removeAttribute('src');
          loadIntoIframe(attemptedSrc);
        }
      }
    });
  });
